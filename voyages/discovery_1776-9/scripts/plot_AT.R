# R script to plot AT time-series for the Discovery
library(grid)
library(chron)
library(auto.tics)

# Read in the data
sts<-read.table('../../../ovn/Discovery_1776-9.ovn',header=F)
sts$Date<-chron(dates=as.character(sts$V1),times=as.character(sts$V2),format=c(dates = "y/m/d", times = "h:m:s"))
sts2<-read.table('../../../ovn/Discovery_1776-9_b.ovn',header=F)
...
pdf(file="../figures/AT.pdf",width=11.7,height=8.3,
...
pushViewport(dataViewport(as.numeric(c(sts$Date[1],sts$Date[length(sts$Date)])),c(min(sts$V7,sts2$V7,na.rm=T),max(sts$V7,sts2$V7,na.rm=T))))
