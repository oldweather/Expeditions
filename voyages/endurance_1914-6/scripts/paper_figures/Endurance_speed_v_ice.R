...
frozen.in<-chron(dates=c("1915/01/18"),
          times=c("12:00:00"),
          format=c(dates = "y/m/d", times = "h:m:s"))

broken.out<-chron(dates=c("1916/04/09"),
          times=c("12:00:00"),
          format=c(dates = "y/m/d", times = "h:m:s"))
...
pushViewport(viewport(width=1.0,height=0.5,x=0.0,y=0.5,
   ...
   pushViewport(plotViewport(margins=c(4,6,0,6)))
      pushViewport(dataViewport(id,c(0,500)))
         ...
         grid.lines(x=unit(rep(frozen.in,2),'native'),
                    y=unit(c(0,500),'native'),
                    gp=gpar(lwd=2))
   ...
   pushViewport(plotViewport(margins=c(4,6,0,6)))
      pushViewport(dataViewport(id,c(0,500)))
         ...
         grid.text('Ship speed (km/day)',x=unit(-4,'lines'),rot=90)
          ...
          grid.lines(x=unit(rep(broken.out,2),'native'),
                    y=unit(c(0,500),'native'),
                    gp=gpar(lwd=2))
