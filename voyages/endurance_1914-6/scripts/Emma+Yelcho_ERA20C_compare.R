# Compare data from Emma and Yelcho and ERA20C
...
o<-IMMA.read('../../../imma/Emma+Yelcho_1916.imma')
...
get.comparisons<-function(i) {
  ...
  t2m<-ERA20C.get.slice.at.hour('air.2m',year,month,day,hour)
  t2m.mean<-GSDF.interpolate.ll(t2m,o$LAT[i],o$LON[i])  
  prmsl<-ERA20C.get.slice.at.hour('prmsl',year,month,day,hour)
  prmsl.mean<-GSDF.interpolate.ll(prmsl,o$LAT[i],o$LON[i])
  sst<-ERA20C.get.slice.at.hour('sst',year,month,day,hour)
  ...
  return(c(t2m.mean,NA,prmsl.mean,NA,sst.mean,NA,icec.mean))
}
...
# Output the result
fileConn<-file(sprintf("Emma+Yelcho.comparisons.ERA20C"))
