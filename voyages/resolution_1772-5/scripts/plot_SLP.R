# R script to plot SLP time-series for the Hecla
library(grid)
library(chron)
library(auto.tics)
...
pushViewport(dataViewport(as.numeric(c(sts2$Date[1],sts$Date[length(sts$Date)])),c(min(sts$V9,sts2$V9,sts3$V9,na.rm=T),max(sts$V9,sts2$V9,sts3$V9,na.rm=T))))
...
grid.text('Sea-level Pressure (hPa)',x=unit(-4,"lines"), rot=90)
   ...
   grid.points(x=unit(sts3$Date,"native"),y=unit(sts3$V9,'native'),pch=20,
               size=unit(5,"native"),gp=sgp)
   sgn = gpar(col=rgb(0,0,0,1),fill=rgb(0,0,0,1),lwx=2)
   grid.lines(x=unit(sts$Date,"native"),y=unit(sts$V10,'native'),gp=sgn)
   sgn = gpar(col=rgb(0,0,0,1),fill=rgb(0,0,0,1),lwx=2)
   grid.lines(x=unit(sts2$Date,"native"),y=unit(sts2$V10,'native'),gp=sgn)
