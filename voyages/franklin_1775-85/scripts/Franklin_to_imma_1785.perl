...
while ( my $Line = <> ) {
    ...
    # Up to 3 obs in each day.

    $Ob->{LAT} = undef;
    $Ob->{LON} = undef;
    $Ob->{HR}  = 6;         # Assume 6 am ob time
    ...
    # Output the morning ob
    $Ob->write( \*STDOUT );

    # put position, wind and supplemental in noon ob.
    ...
    $Ob->{LAT} = undef;
    $Ob->{LON} = undef;
    $Ob->{HR}  = 18;      # Assume 6 p.m. measurement time
    correct_hour_for_lon($Ob);
