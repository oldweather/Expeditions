...
# Process the digitised Franklin data from the 1775 trip into
#  IMMA records.
...
while ( my $Line = <> ) {
    ...
    # Date
    $Ob->{YR} = 1775;
    if ( $Fields[0] eq 'April' ) { $Month = 4; }
    if ( $Fields[0] eq 'May' )   { $Month = 5; }
    $Ob->{MO} = $Month;
    ...
    # Latitude
    if ( $Fields[3] =~ /\d+/ ) {
        $Ob->{LAT} = $Fields[3] + $Fields[4] / 60;
    }

    # Longitude
    if ( $Fields[5] =~ /\d+/ ) {
        $Ob->{LON} = $Fields[5] + $Fields[6] / 60;
        $Ob->{LON} *= -1;
        $Last_lon = $Ob->{LON};
    }
    if ( $Fields[2] =~ /(\d+)(.m)/ ) {
    ...
    # Wind direction
    if ( $Fields[9] =~ /\S/ ) {
        $Ob->{D}  = decode_direction( $Fields[9] );
